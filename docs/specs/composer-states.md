...
**One phase, ten values, every surface reads it.** `closed · rest · typing · pending · listening · cancelled · denied · unsupported · lost · leaving` (a `stopped` phase existed for a day and folded into `leaving`), derived once from the mic hook's own state (`off · pending · on · stopped · cancelled · denied · unsupported · lost`) plus `open` and focus. Listening comes from the hook, never from the click, so a refusal can never flash a face. Adding a phase without all six columns below is the regression.
...
| leaving | quiet dots · teal | plain | You were talking. Go on, keep it, or close? · You have a request here. Keep it, or close? | stopped — `go on` · `keep it` · `close` at the field's right |

**One control (Lindsay, 2026-09-06: "with the scope reduction is the microphone needed now?" — it was not).** The mic glyph in the field duplicated the grip and was the one generic thing left in the frame; what it uniquely gave, *stop without leaving*, is absorbed by the leaving question. So: press → open, listening. Press → the mic pauses and the field asks "You were talking. Keep going, or close?" — the question is what makes the second press legible, which is why the earlier unlabelled cycle felt odd and this does not. `close` or a third press → closed. From a fallback text state with nothing typed, a press listens again; with a request there, it asks. The field is the grip's caption. Grip label: *open the composer and listen* / *stop listening* / *close the composer*.

**Listening again (Lindsay, 2026-09-08).** After `go on` the field says "Listening again. In the full build your words would land here. Not this one." (casual on purpose: it names the missing transcript as a choice, with a shrug) — by the second listen the viewer has noticed no transcript appeared, and the honest answer goes where they are looking. (The missing thing is a transcript; the feature is live transcription, speech-to-text.) The caption carries the same fact as a fourth line, **Not**.

**Leaving (Lindsay, 2026-09-06: "there is no recognition… then offer a continue or quit").** Closing is the one move that throws something away, so it asks first — only when there is something, and only once; the second press is the answer. Mid-sentence: the mic pauses and the field says "You were talking. Go on, keep it, or close?" — `keep it` turns what you said into a request pill (kind `heard`, dashed, muted, at helper size, reading `your request`: it holds no words, and the caption already says this one does not write things down, so the field adds nothing; it is inert — no click, no Backspace, only closing takes it, because a stub you can dismiss to nowhere is a trapdoor, Lindsay 2026-09-08) That pill is the same object typing makes and the unit the composer learns from — repeated over a person, a team, an org it becomes their chips (T/02). A save that saves nothing was considered and refused (Lindsay, 2026-09-06). With typed words or a typed request: "You were typing. Keep typing, or close?" — a spoken stub alone has nothing to lose, so the grip just closes Already stopped with the glyph, or empty: it just closes — you ended it, there is nothing to ask. Answers are mono tags at the field's right, in your voice: voice `go on` · `keep it` · `close`; text `keep typing` · `close`. No verb is reused across the two questions. The question sits in the box unless your own words are there, in which case it sits beneath. Phase `leaving`; the status tag reads `stopped`.

**Pressable without a pointer.** The teal status dot at rest is the cue; hover and the keyboard ring turn that one dot orange (only where hover exists, so a phone tap does not stick). All the dots orange is reserved for doing something — otherwise a grip you just pressed to close looks like it is still listening (Lindsay, 2026-09-08). Escape stops listening, else closes.
...
**Voice or text, never both (Lindsay, 2026-09-06: "if it is only listening then the pill and type input is like, what is happening").** While it asks, listens, or asks whether to leave, the field is a single sentence — no text box, no caret, no pills. In every other open phase the text box and pills come back, because the keyboard is the fallback when the mic cannot be used. Starting to listen always begins from empty, which is what "nothing you said was saved" already promises.
