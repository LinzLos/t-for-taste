...
**Node padding (Lindsay, in Figma):** 32 on all sides, was 22/24. Matched in `builder.css`. Inner gap stays 6.
**Card height parity:** Figma's Node hugs content and lands at exactly 200 with padding 32. The code card ran ~239 because browser default line heights for Fredoka and Courier Prime are taller than Figma's auto. Code line heights are now pinned to Figma's metrics (type 20, title 36, sub 29, pill text 21), so both cards are 200.
**T/NN mark removed from the stage (Lindsay).** The recording carries no series mark in-frame; the episode number lives in the chassis and the post caption. `Mark.tsx` stays in the chassis for reuse.

## Anthropomorphism check + dynamic placeholder (2026-09-02, Lindsay's question)
**Test (hers, from the freewrite):** it is a tool. A tool reports, observes, offers; it never feels and never reads your feelings. Clean: all system log lines and review findings ("It moved your page"), the system is always "it", never "I". Clean by convention: "Fuck this, show me" is the user's voice on a button; Marike swears, the system doesn't. Borderline: "Had enough?" reads her state from six identical tries (Netflix's "Still watching?" move); kept for now, her call; the tool-side alternative is "Six tries, same answer. See what happened while you clicked?" Over the line: "Marike, what should change?" in the input box (companion cosplay). Removed.
**Dynamic placeholder:** the composer follows the state. calm: "Describe a change" · after try 3: "Still 0. Where should it look?" · after try 4: "Try a different page, or tell it where to look" · threshold: "Reconnecting has not changed the answer". Fades in on change. No name, no "I".

## Fluid on desktop, fixed only for the recording (2026-09-02, Lindsay: "on my Air it looks small and tiny; what happens on a studio; then it scales for mobile")
**Principle:** it is a working prototype. On a desktop it should behave like a desktop app and fill the window; the 4:5 social frame is what you get by resizing the window or opening `?record`. Constraining the story to a scaled-down 4:5 box on a laptop made it small for no reason.
**Built:**
- **Stage** has two modes. Fluid (default): the stage is the host, no scaling. Record: fixed 1080 × 1350 scaled to fit, unchanged for recordings.
- **Builder** is now a real layout: grid (top bar / rail / main), cards positioned by percentage of the canvas, connectors re-measure on resize (ResizeObserver) and divide by any transform scale, the review fills the main area. A container query flips the main area: portrait stacks canvas over chat; **landscape puts the chat as a side panel** (42%, max 560px) beside the canvas, the way real builders do it.
- **Cursor and melt** measure everything live in the builder's own pixels (`spotOf` with the transform scale factored out), so play works in fluid and record modes alike; the melt distance is the canvas height.
**Open:** type sizes inside the builder are still fixed px, so on a studio display the cards are the same size as on the Air, just with more room. If the story should scale up on big screens, that's a `cqw`-based type pass, and it should be decided together with the social-legibility pass.
**Follow-up:** fluid only above 900px wide; below that (phones, narrow tablets) the scaled 4:5 stage stays, since a fluid layout has no room there. The fluid host keeps a real height (min 760px) so the stage can't collapse; cards clamp inside the canvas (`min(x%, calc(100% − card))`) so a narrow landscape canvas can't push one off the edge; landscape canvas has a 520px floor so cards don't stack on each other.
**Follow-up 2:** landscape rules were losing to the base rules on specificity order (moved to the end of the file); the fluid decision now comes from the stage host's own measured width via ResizeObserver (860px), not a window media query, so it flips correctly on any resize and on first mount.
**Follow-up 3 (Lindsay: "looks bad, all jumbled"):** percentage-positioned cards with fixed widths piled up on a window that was fluid-wide but canvas-narrow. Wrong tool. The canvas is a composed picture, so it now scales as one: a 1008 × 800 `.scene` holds the cards and wires at their designed positions and scales to fit whatever box the window gives the canvas (ResizeObserver, min of width/height ratios). The chat panel stays fluid because it flows. Fluid threshold raised to 1000px body width; below it the scaled 4:5 stage. Cursor, pulses and melt all measure inside the scene, so play still works.

## Pulse and side panel (2026-09-02, Lindsay's notes on the Air)
- **The pulse reaches the card.** It used to stop at 42% to say "nothing arrives"; a wire should deliver. It now travels the whole wire (0.9s, power1.inOut) and fades on contact, and Summarize doesn't react. Same story.
- **The pulse is Lotion's heartbeat.** Each emission, the Lotion card beats: a teal ring expands 22px off its edge over 0.8s and the card lifts 2.5% for 120ms, then the dot leaves its edge. Hot on a rage tap.
- **The side panel was giant on the Air.** In landscape the chat is now a real side chat: 34% wide, 320–440px, with smaller type (messages 19, system 16, field 19, offer 22/18, buttons 17) and tighter padding. Portrait and record keep the big social-legible type.
**Duplo chrome (Lindsay):** in the fluid app window the top bar, chips, Publish, rail and review were still at social-stage size. Fluid mode now draws the chrome at app size (project 20, chips 14, Publish 24, rail 56 with 20px icons, review type ~two-thirds). Portrait/record keep the big sizes. The cards are untouched; they live in the scaled scene.
**Correction (Lindsay: "no blur pulse, I mean the change between corner radii can be MORE"):** the glow ring and card lift are removed. The trying card's breathing now swings 52 → 4 → 52 over 1.8s, so it visibly firms and relaxes. Study strip note and meta values updated.
...
**Ellipsis, second pass (Lindsay: too far apart, too fast, too noticeable next to a very movement-y workflow):** dots tightened (letter-spacing −.12em, 1.2ch wide, 85% opacity), cycle slowed from 1.2s to 3.2s, and the timing is uneven: a beat of nothing, then the dots arrive one by one, then the full ellipsis lingers for the back half. Reads as a pause, not a ticker.
**Cards and side chat, again (Lindsay's Air screenshot):** the Figma-pinned line heights read cramped in the browser; loosened (type 24, title 40, sub 32, pill 24, gap 10) so the card is ~224 instead of 200, a trade for legibility. The landscape side chat shrinks again: 30% / 300–400px, messages 16, system 14, field 16, offer 19/15, thinner 1.5px borders.
**Chat in the app window, third pass (Lindsay: still big and clunky):** chat sizes are now keyed on fluid mode, not aspect: messages 14, system 12.5, field 14, offer title 16 / body 13.5 / buttons 13 on one line (nowrap), panel 300–380px. Pills never wrap ("still trying · 9 min" was breaking onto two lines); pill type 20. Social frame and record keep the big type.
...
**Later, 2026-09-06 · Lindsay on 5181:** the EQ "reads like static," the faded dots contribute; reference frame 186:6013 and LED-matrix stills (bars from the bottom, a lone peak dot, unlit = off). **Agent:** discrete on/off dots, no floor; attack/release ballistics (25/180ms); neighbour smoothing; peak hold 450ms falling 120ms/row; 1024-point FFT with nine log bands and a high tilt; 80ms on/off ease; unroll is grid → line → meter. Her call: try three rows before adding dots.

**Review pass, 2026-09-06 (Lindsay: "a clean and pro bug sweep, make it fresh and clean").** Eight review angles, one verifier. Real bugs fixed: mic left open on reset/replay; listening face flashing on refusal; retry stuck salmon; first 320ms of speech dropped during the unroll; unhandled `resume()` rejection; replay ending with nothing built. Cleanups: one derived `phase`, one `reset()`, one `EASE`, allocation-free mic tick, line height measured once, `:where()` for hover, `yours` outline restored, `asYours` whitespace normalised, `project` read from `pickOn`. Spec brought back in line with her calls (state copy in the field; gripper top left).

**Lindsay, 2026-09-06:** click-anywhere-to-focus is misleading; the grip must be the way in — hover shows pressable, press opens the composer listening with the mic orange; title `friendly voice composer`; are the binding chips baggage? **Agent:** grip is a button (hover orange via `:where`), `open` state set only by the grip (and replay), root click no longer focuses, mic starts inside the grip's click; bindings parked with the picker, status tag kept as the narrator. Verified closed/opened headless and the refused path in the pane.

**Lindsay, 2026-09-06 (later):** press opens listening but the underline was missing; wants press → stop → close on the grip; the meter pins at the top when talking constantly; the copy after stopping confuses; the tall frame is empty; must look on all devices; use FLOWIE. **FLOWIE (friction lens):** 5 invariant failures, a full state table, the contract. **Agent:** one phase with nine values folded from the hook (`stopped · cancelled · unsupported` added to `MicState`, `dismiss()` on keystroke), the three-press cycle with `close()` separate from `reset()`, grip `open` look + teal resting dot + focus-visible + hover only where hover exists, copy that says where the action is, refused ≠ unsupported, ▶ play enters through the grip, Escape, an adaptive per-band ceiling so constant talk spreads across rows (thresholds 0.22 / 0.62 against it), per-episode `stage` (square) and `fluidMin` (0) in the chassis, container-query sizing for narrow widths. Verified: the cycle and refused path in the pane, mobile fluid layout at 375px, square record frame headless.

**Lindsay, 2026-09-06 (later still):** name the states, what's missing, what are we saying; add `lost` and suspended handling; the typing door goes nowhere; "the type persists!!". **Agent:** `lost` state (track `ended`, context `statechange`, visibility-revive with fallback to lost); typing ends at a request pill (build and the built line gated to `?chips`); close is a clean slate. Verified in the pane by driving the DOM: type → Enter → one pill, Enter on empty → nothing, close → reopen → empty.

**Lindsay, 2026-09-06 (screenshot on 5181):** the open look reads as a weird white border; after listening the mic glyph should change into confirm/close so the states read and the viewer knows it is not connected to data. **Agent:** plate stroke gone, open = quiet dots; `stopped` swaps the glyph for a teal check labelled *done* that closes; copy states the fact: "this one listens, it does not transcribe."
...
**Lindsay, 2026-09-06:** is there a third option — save? "enough stub to be imagined used" without falseness; "make a request fits our overall schema for the chunked-out composer strings that are AI-filled AND responsive to repeated requests over a user/team/org." **Agent:** not save (it would save nothing) — `keep it`: the spoken sentence becomes a `heard` request pill that admits it holds no words; the hand-off object to the chips story.
...
**Lindsay, 2026-09-08 (user feedback):** mic tell back in the field; the dots could spell the action; the status chip looks pressable; a line between dots and smile and a doubled edge under the bar. **Agent:** status is a coloured word (all tones ≥4.5:1); grip in the field's language (one plate, bottom edge follows state); mic as a non-pressable tell; option B `?word` — PRESS/TALK in a 27×5 matrix that falls into a finer meter on the first sound.
